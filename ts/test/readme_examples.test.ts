...
//     (\`new IinLookupSDK(...)\` / \`IinLookupSDK.test(...)\` become
//     \`IinLookupSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"overview":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new IinLookupSDK(...)\` and any
// \`IinLookupSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+IinLookupSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/IinLookupSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
