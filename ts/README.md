...
`client.Overview()` — each with a small set of operations (`load`, `create`)
...
| `tester(testopts?, sdkopts?)` | `IinLookupSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `IinLookupSDK.test(testopts?, sdkopts?)` | `IinLookupSDK` | Create a test-mode client. |
...
| `client` | `client(): IinLookupSDK` | Return the parent SDK client. |
...
iin-lookup/
├── src/
│   ├── IinLookupSDK.ts        # Main SDK class
