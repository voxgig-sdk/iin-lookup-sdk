...
import (
	...
	omni "github.com/voxgig-sdk/iin-lookup-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.IinLookupError); is {
				out := map[string]any{
					"name":    "IinLookupError",
...
func omniCtx(arg any, client *sdk.IinLookupSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.IinLookupSDK); is {
			client = live
