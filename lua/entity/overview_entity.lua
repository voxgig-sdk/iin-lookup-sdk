...
local OverviewEntity = {}
OverviewEntity.__index = OverviewEntity


function OverviewEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, OverviewEntity)
  self._name = "overview"
  self._client = client
...
function OverviewEntity:get_name()
  return self._name
end


function OverviewEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return OverviewEntity.new(self._client, opts)
end
...
function OverviewEntity:mark_deleted()
  self._deleted = true
end


function OverviewEntity:deleted()
  return true == self._deleted
end


function OverviewEntity:data_set(args)
  if args ~= nil then
...
function OverviewEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function OverviewEntity:match_set(args)
  if args ~= nil then
...
function OverviewEntity:match_get()
...
function OverviewEntity:stream(action, args, callopts)
...
function OverviewEntity:_run_op(ctx, post_done)
  local utility = self._utility
