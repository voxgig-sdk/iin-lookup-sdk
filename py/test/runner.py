...
class IinLookupTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            IinLookupTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = IinLookupTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = IinLookupTestRunner.getenv("IIN_LOOKUP_TEST_LIVE")
        override = IinLookupTestRunner.getenv("IIN_LOOKUP_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = IinLookupTestRunner.getenv(key)
        ...
        explain = IinLookupTestRunner.getenv("IIN_LOOKUP_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["IIN_LOOKUP_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if IinLookupTestRunner._test_control is not None:
            return IinLookupTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                IinLookupTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            IinLookupTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = IinLookupTestRunner.load_test_control()
...
def load_env_local():
    IinLookupTestRunner.load_env_local()


def env_override(m):
    return IinLookupTestRunner.env_override(m)


def entity_data(v):
    return IinLookupTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return IinLookupTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return IinLookupTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return IinLookupTestRunner.load_test_control()


def live_client_options():
    return IinLookupTestRunner.live_client_options()


def live_delay_ms():
    return IinLookupTestRunner.live_delay_ms()
