...
from iinlookup_sdk.core.control import IinLookupControl
...
from iinlookup_sdk.core.helpers import get_ctx_prop, to_map
...
class IinLookupContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, IinLookupSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, IinLookupResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, IinLookupResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return IinLookupError(code, msg, self)
