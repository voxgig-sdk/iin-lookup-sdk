...
[MCP](https://modelcontextprotocol.io) server exposing the IinLookup SDK as
two agent tools — `iin-lookup_list` and `iin-lookup_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/iin-lookup-mcp)
make build

# 2. Provide credentials via the environment
export IIN_LOOKUP_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user iin-lookup \
  -- /absolute/path/to/iin-lookup-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./iin-lookup-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// iin-lookup_list: first page of records
{ "entity": "overview" }
{ "entity": "overview", "query": { } }

// iin-lookup_load: one record by id
{ "entity": "overview", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/iin-lookup-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user iin-lookup \
     -- "$PWD"/dist/*/iin-lookup-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `iin-lookup_list` and `iin-lookup_load` tools now appear
   in new sessions. Ask the agent to *"list overview using iin-lookup"*
   and it calls `iin-lookup_list` with `{"entity":"overview"}`.
...
```sh
export IIN_LOOKUP_APIKEY=sk_live_xxx            # API key
export IIN_LOOKUP_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `iin-lookup_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `iin-lookup_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `IIN_LOOKUP_APIKEY` | API key sent with every request. |
| `IIN_LOOKUP_BASE` | Optional override of the API base URL. |
...
```sh
./iin-lookup-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"iin-lookup_load","arguments":{"entity":"overview","query":{"id":1}}}}'
```
