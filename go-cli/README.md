...
boru-driven command-line client **and** interactive REPL for the IinLookup
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/iin-lookup-cli)
make build
...
./iin-lookup-cli load 1 overview            # {id:1} shorthand
./iin-lookup-cli load '{id:1}' overview       # explicit match map

# 5. Override the API base URL for a single call
IIN_LOOKUP_BASE=https://api.example.com ./iin-lookup-cli load 1 overview

# 6. No arguments -> interactive REPL
./iin-lookup-cli
iin-lookup> load 1 overview
iin-lookup> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/iin-lookup-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/iin-lookup-cli load 1 overview
   ```
...
```sh
./iin-lookup-cli load 1 overview          # scalar shorthand for {id:1}
./iin-lookup-cli load '{id:1}' overview     # explicit match map
```
...
./iin-lookup-cli load 1 overview
```
...
Run with no arguments to open a REPL (prompt `iin-lookup>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `overview`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/iin-lookup-cli`. |
...
(`./iin-lookup-cli <expr>`) and interactively (the REPL), and expressions compose the
