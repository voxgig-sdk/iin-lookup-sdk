...
class IinLookupRetryFeature extends IinLookupBaseFeature
{
    ...
    public function init(IinLookupContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (IinLookupContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(IinLookupContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(IinLookupContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
