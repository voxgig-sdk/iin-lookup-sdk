...
class FtCtrl extends IinLookupControl
{
...
class FtClient
{
    ...
    private IinLookupUtility $utility;

    public function __construct(IinLookupUtility $utility, array $options)
    {
    ...
    public function get_utility(): IinLookupUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (IinLookupContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public IinLookupContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (IinLookupContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = IinLookupConfig::make_feature($name);
    ...
    private static function buildUrl(IinLookupSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof IinLookupError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new IinLookupResponse($fetched);
        }
    ...
    private function fail(IinLookupContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (IinLookupContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
