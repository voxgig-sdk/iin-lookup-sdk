...
{
    private static function utility(): IinLookupUtility
    {
        return new IinLookupUtility();
    }
    ...
    private static function ctx(array $over = []): IinLookupContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new IinLookupContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new IinLookupOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof IinLookupError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new IinLookupOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = IinLookupMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new IinLookupOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = IinLookupMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new IinLookupOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = IinLookupMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new IinLookupSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = IinLookupMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new IinLookupResponse([]),
            'result' => new IinLookupResult([]),
        ]);
        [, $err] = IinLookupMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new IinLookupSpec([]),
            'response' => null,
            'result' => new IinLookupResult([]),
        ]);
        [, $err] = IinLookupMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new IinLookupSpec(['step' => 's']),
            'response' => new IinLookupResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new IinLookupResult([]),
        ]);
        [, $err] = IinLookupMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new IinLookupSpec(['step' => 's']),
            'response' => new IinLookupResponse(self::resp(200, ['v' => 1])),
            'result' => new IinLookupResult([]),
        ]);
        [, $err] = IinLookupMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new IinLookupSpec(['step' => 's']),
            'response' => new IinLookupResponse(self::resp(200, ['v' => 2])),
            'result' => new IinLookupResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        IinLookupMakeResponse::call($ctx);
    ...
    {
        $preset = new IinLookupResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new IinLookupSpec([]),
            'response' => new IinLookupResponse([]),
            'result' => new IinLookupResult([]),
        ...
        [$got, $err] = IinLookupMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new IinLookupResult([])]);
        [, $err] = IinLookupMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new IinLookupSpec([]), 'result' => null]);
        [, $err] = IinLookupMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new IinLookupSpec(['step' => 's']),
            'result' => new IinLookupResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new IinLookupOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = IinLookupMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new IinLookupSpec(['step' => 's']),
            'result' => new IinLookupResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new IinLookupOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = IinLookupMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new IinLookupResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new IinLookupSpec([]),
            'result' => new IinLookupResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = IinLookupMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = IinLookupMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new IinLookupError('boom', 'boom');
        $utility->fetcher = function (IinLookupContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new IinLookupSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = IinLookupMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (IinLookupContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = IinLookupMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(IinLookupResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (IinLookupContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new IinLookupSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = IinLookupMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new IinLookupResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new IinLookupSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = IinLookupMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = IinLookupMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = IinLookupMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new IinLookupResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, IinLookupDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new IinLookupResult(['ok' => false])]);
        $this->expectException(IinLookupError::class);
        IinLookupDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new IinLookupResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', IinLookupMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        IinLookupMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new IinLookupBaseFeature();
        $b = new IinLookupBaseFeature();
        IinLookupFeatureAdd::call($ctx, $a);
        IinLookupFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): IinLookupBaseFeature
    {
        $f = new IinLookupBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        IinLookupFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): IinLookupContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new IinLookupSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = IinLookupPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new IinLookupResponse(['headers' => null]),
            'result' => new IinLookupResult([]),
        ]);
        IinLookupResultHeaders::call($ctx);
