...
// IinLookup SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = IinLookupOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(IinLookupOmni::structfix($now, Struct::undef()))
                        !== json_encode(IinLookupOmni::structfix($val, Struct::undef()))) {
