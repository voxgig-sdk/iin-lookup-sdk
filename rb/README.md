...
The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Overview` — with named operations (`load`/`create`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = IinLookupSDK.test(testopts, sdkopts)
```
...
raise a `IinLookupError` (a `StandardError` subclass), so wrap
...
```
rb/
├── IinLookup_sdk.rb       -- Main SDK module
...
The main module (`IinLookup_sdk`) exports the SDK class
