...
module IinLookupUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = IinLookupResponse.new({})
    ...
    if fetch_err
      ...
      response = IinLookupResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = IinLookupResponse.new(fetched)
    else
