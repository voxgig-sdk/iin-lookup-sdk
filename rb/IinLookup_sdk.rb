...
require_relative 'IinLookup_types'
...
class IinLookupSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = IinLookupHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = IinLookupHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, IinLookupFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    IinLookupUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = IinLookupHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = IinLookupHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = IinLookupHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = IinLookupHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = IinLookupError.new(
        "graphql_error", "IinLookupSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
