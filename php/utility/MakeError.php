...
class IinLookupMakeError
{
    public static function call(?IinLookupContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new IinLookupContext([], null);
        }
        $op = $ctx->op ?? new IinLookupOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new IinLookupResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof IinLookupError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new IinLookupError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof IinLookupError) {
