...
class OverviewEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set IIN_LOOKUP_TEST_OVERVIEW_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $overview_ref01_data = Helpers::to_map(is_object($overview_ref01_data_result) && method_exists($overview_ref01_data_result, 'data_get') ? $overview_ref01_data_result->data_get() : $overview_ref01_data_result);
...
function overview_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("IIN_LOOKUP_TEST_OVERVIEW_ENTID");
