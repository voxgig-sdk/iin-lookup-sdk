...
class IinLookupSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = IinLookupHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = IinLookupHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!IinLookupFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, IinLookupFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return IinLookupUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = IinLookupHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = IinLookupHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = IinLookupHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new IinLookupError($op . "_allow",
                "IinLookupSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = IinLookupHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new IinLookupError("graphql_error",
                "IinLookupSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Overview($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_overview === null) {
                $this->_overview = new OverviewEntity($this, null);
            }
            return $this->_overview;
        }
        return new OverviewEntity($this, $data);
    }
