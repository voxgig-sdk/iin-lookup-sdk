...
class OverviewEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new OverviewEntity($this->_client, $opts);
    }

    /**
     * @param Overview|array $args Overview data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = IinLookupHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Overview|array The current Overview data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Overview fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = IinLookupHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Overview fields).
     */
     ...
     *
     * @param OverviewLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed OverviewLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Overview|array The loaded Overview as an assoc-array at the
     *   SDK boundary; throws IinLookupError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = IinLookupHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *
     * @param OverviewCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed OverviewCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Overview|array The created Overview as an assoc-array at the
     *   SDK boundary; throws IinLookupError on failure (item-5 convention).
     */
